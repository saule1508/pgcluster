                ...
                <td>
                  ...
                  >
                    <button
                      ...
                      style={{ marginRight: 5 }}
                      onClick={onClickDelete.bind(null, el, host)}
                      disabled={processing}
                    >
                      Delete
                    </button>
                    <button
                      className="btn btn-default"
                      onClick={onClickRestore.bind(null, el, host, "restore")}
                      disabled={processing}
                    >
                      Restore
                    </button>
...
class Backup extends Component {
  ...
  onClickRestore(buname, host, action, evt) {
    evt.preventDefault();
  ...
  renderRestoreModal() {
    ...
    return (
      <RestoreModal
        handleHideModal={this.stopRestoreModal.bind(this)}
        restoreModalActive={this.state.restoreModalVisible}
        onSubmit={this.onSubmitRestore.bind(this)}
        name={this.state.buname}
  ...
  render() {
      ...
      if (data.hasOwnProperty(el)) {
        hostelems.push(
          <Backups
            ...
            } /* so that we can disable buttons */
            rows={data[el].result}
            error={data[el].error}
    ...
    console.log(args);
    let backupSubmitEnabled =
      this.state.restoreModalVisible || this.state.processing ? false : true;
    return (
      <div>
        <div className="row">
          <div className="col-md-12">
            <h3>Backups</h3>
            <div className="row">{data && hostelems}</div>
          </div>
        </div>
        ...
        <div className="row">
          <div className="col-md-6">
            <BackupForm
              name={this.state.name}
              enabled={backupSubmitEnabled}
              setName={this.setName}
              hosts={hosts ? hosts : []}
              onSubmit={this.onSubmitBackup}
            />
          </div>
        </div>
        <div className="row">
