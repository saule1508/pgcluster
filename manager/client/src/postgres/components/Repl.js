  ...
  let strokeW = backend.type === 'primary' ? 5 : 2;
	...
	return (
		...
		<svg width="130" height="100">
			...
			<rect width={120} height={100} style={{fill : color,strokeWidth: strokeW, stroke: strokeC}} />
			<text x={30} y={20} style={{fontSize: '110%'}} fill={strokeC}>** {backend.type} **</text>
			<text x={10} y={40} style={{fontSize: '80%'}} fill='black'>
				ID: {backend.node_id} - Host:{backend.node_name}
        <tspan x={5} y={60}>State:{backend.active ? 'active' :'inactive'}</tspan>
				<tspan x={5} y={70}>{backend.slot_name}</tspan>
				<tspan x={5} y={80}>Upstream node id: {backend.upstream_node_id}</tspan>
			</text>
		</svg>
	)
